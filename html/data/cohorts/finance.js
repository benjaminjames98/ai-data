...
/**
 * Sets up the lower half of the page to display the information about a
 * particular payee. A lot of the work is delegated to other functions. Also
 * sets up crt and save modal buttons.
 *
...
function createInvoice(id) {
  ...
  for (var i = 0; i < chgArr.length; i++)
    if (chgArr[i].split('>').length === 3) {
      alert(
        'Looks like you haven\'t formatted everything quite right. Have another look');
      return;
    }
 ...
 *
 * @param mdl   The modal to be prepared. Each is prepared using custom code
 *   based on this value.
