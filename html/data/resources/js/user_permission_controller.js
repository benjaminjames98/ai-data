const UserPermissionController = function (select_el, container_el, button_el) {
  ...
  function loadPermissions(uid = "") {
    username = uid;
    jsonPost(_EDIT_PER_URL, {q: 'read_permissions', username: username}, (obj) => {
      container_el.innerHTML = `<p>${obj.username}</p>`;
      let reducer = (acc, per) => acc
        + `<label for="${per}"><input type="checkbox" id="${per}" ${obj.permissions[per] ? 'checked' : ''}>${per}</label><br>`;
      container_el.innerHTML += Object.keys(obj.permissions).reduce(reducer, '');
    });
  }
  ...
  button_el.onclick = function () {
    let perms = {};
    container_el.querySelectorAll('input').forEach((input) => perms[input.id] = input.checked);
    jsonPost(_EDIT_PER_URL, {q: 'update_permissions', username: username, permissions: perms}, (obj) => {
