...
MYSQL;
    ...
    $stmt->execute();
    $stmt->store_result();
    if ($stmt->num_rows === 0) die(json_encode(["success" => true, 'region' => []]));
...
MYSQL;
    ...
    while ($stmt->fetch())
        $region['routes'][$tid] = ['id' => $tid, 'name' => $tname, 'driver_mac' => $tmac];
    $stmt->close();
...
MYSQL;
    ...
    foreach ($region['routes'] as $route) {
        ...
        $stmt->execute();
        $stmt->bind_result($cid, $cname, $cden, $ctime, $cpar, $cdel, $cln1, $cln2, $cpc,
            $csub, $cstt, $cnot, $clof, $cbun, $cgf);
        while ($stmt->fetch())
            $region['routes'][$route['id']]['churches'][$cid] = ['id' => $cid, 'name' => $cname,
                'denomination' => $cden, 'time' => $ctime, 'participating' => $cpar, 'delivered' => $cdel,
                'line_1' => $cln1, 'line_2' => $cln2, 'post_code' => $cpc, 'suburb' => $csub,
                'state' => $cstt, 'note' => $cnot, 'loaves' => $clof, 'buns' => $cbun,
                'gluten_free' => $cgf];
    }
