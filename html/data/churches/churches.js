class View {
  ...
  updateFilters(entities, cp = 'c') {
    if (cp === 'p') {
      ...
      $.each(entities, function(i, ent) {
        if ($.inArray(ent['denomination'], den) === -1) den.push(
          ent['denomination']);
        if ($.inArray(ent['visibility'], vis) === -1) vis.push(
          ent['visibility']);
    ...
    function createCheckboxes(container, title, arr, attr) {
      let outerDiv = $('<div>',
        {'class': 'w3-bar-item w3-border-bottom w3-animate-left'}).append(
        `<h5>${title}</h5>`).append('<div class=\'switch\'>');
  ...
  scrollTo(id) {
    ...
    let delta = scrollTo.offset().top - container.offset().top
      + container.scrollTop();
  ...
  addRolesRow(ctc, ent, people_table, cp) {
    let row = $('<tr>');
    let role = $('<td>').css({'width': '30%', 'word-wrap': 'break-word'}).html(
      ctc['role']);
    let see = $('<td>').css('width', '45px').attr(
      {'class': 'w3-button', 'title': 'edit contact'});
    see.append($('<i>', {'class': {c: 'fa fa-user', p: 'fa fa-church'}[cp]}));
    let name = $('<td>').css({'word-wrap': 'break-word'}).html(ctc['name']);
    let edit = $('<td>').css('width', '30px').attr(
      {'class': 'w3-button', 'title': 'edit contact'}).append(
      $('<i class=\'fas fa-pencil-alt\'>'));
    let del = $('<td>').css('width', '30px').attr(
      {'class': 'w3-button', 'title': 'delete contact'}).append(
  ...
  addContactRow(ctc, id, contacts_table, cp) {
    ...
    let row = $('<tr>');
    let icon = $('<td>').css('width', '40px').append(
      $('<i>').attr({'class': icons[ctc['type']]}));
    let label = $('<td>').css({'word-wrap': 'break-word'}).html(ctc['contact']);
    let edit = $('<td>').css('width', '30px').attr(
      {'class': 'w3-button', 'title': 'edit contact'}).append(
      $('<i class=\'fas fa-pencil-alt\'>'));
    let del = $('<td>').css('width', '30px').attr(
...
class Model {
  ...
  constructor(view) {
    this._url = '../resources/ajax_backend/churches_backend.php'; // URL of the
                                                                  // php
                                                                  // webpage
  ...
  createContact(type, church_id, cp, ctc1, ctc2, ctc3, ctc4, ctc5) {
    let obj = null;
    if (type === 'web') obj =
      {'type': type, 'id': church_id, 'cp': cp, 'url': ctc1};
    else if (type === 'eml') obj =
      {'type': type, 'id': church_id, 'cp': cp, 'eml': ctc1};
    else if (type === 'phn') obj =
      {'type': type, 'id': church_id, 'cp': cp, 'phn': ctc1};
  ...
  updateContact(type, contact_id, ent_id, cp, ctc1, ctc2, ctc3, ctc4, ctc5) {
    ...
    if (type === 'web') obj = {'type': type, 'id': contact_id, 'url': ctc1};
      ...
      {'type': type, 'id': contact_id, 'eml': ctc1};
    else if (type === 'phn') obj =
      {'type': type, 'id': contact_id, 'phn': ctc1};
    else if (type === 'add') obj = {
...
function setMetaListeners() {
  ...
  $('#church_create').off().click(() => {
    clearModal();
    let content = $('<div>').append(createInput('Name:', 'mdl_ipt_1')).append(
      createInput('Denomination', 'mdl_ipt_2')).append(
      createRegionSelector('mdl_ipt_3')).append(
      createVisibilitySelector('mdl_ipt_4'));
  ...
  $('#person_create').off().click(() => {
    clearModal();
    let content = $('<div>').append(createInput('First Name:', 'mdl_ipt_1')).
...
function setCreateContactListeners(id, cp) {
  ...
  $('#church_address').off().click(() => {
    ...
    let div = $('<div>');
    div.append(createInput('Line 1', 'mdl_ipt_1')).append(
      createInput('Line 2', 'mdl_ipt_2')).append(
      createInput('Suburb', 'mdl_ipt_3')).append(
      createStateSelector('mdl_ipt_4')).append(
      createInput('Suburb', 'mdl_ipt_5'));
...
function setUpdateContactListeners(btn, ctc, ent_id, cp) {
  ...
  btn.off().click(() => {
    ...
    if (['phn', 'eml', 'web'].includes(type)) {
      ...
      let div = $('<div>').append(
        createInput('Line 1:', 'mdl_ipt_1', ctc['ln1'])).append(
        createInput('Line 2:', 'mdl_ipt_2', ctc['ln2'])).append(
        createInput('Suburb:', 'mdl_ipt_3', ctc['sub'])).append(
        createStateSelector('mdl_ipt_4', ctc['stt'])).append(
        createInput('Post Code:', 'mdl_ipt_5', ctc['pcd']));
      ...
      createModal('Edit ' + labels[type], div, () => {
        ...
        if (ln1 === '' || sub === '' || pcd === '')
          ...
          _MODEL.updateContact('add', ctc['id'], ent_id, cp, ln1, ln2, sub, stt,
            pcd);
          return true;
        }
      });
...
function createInput(label, id = 'mdl_ipt_1', val = '') {
  return $('<label class=\'w3-bar-item\'>').html(label).append(
    $('<input >').val(val).attr({
...
function createStateSelector(id = 'mdl_ipt_1', val = '') {
  let select = $('<select>').attr({'id': id, 'name': id, 'class': 'w3-input'}).
    append($(`<option value='TAS'>Tasmania</option>`)).append(
      $(`<option value='VIC'>Victoria</option>`)).append(
      $(`<option value='ACT'>Australia Capital Territory</option>`)).append(
      $(`<option value='NSW'>New South Wales</option>`)).append(
      $(`<option value='QLD'>Queensland</option>`)).append(
      $(`<option value='SA'>South Australia</option>`)).append(
      $(`<option value='NT'>Northern Territory</option>`)).append(
      $(`<option value='WA'>Western Australia</option>`));
  return $('<label class=\'w3-bar-item\'>').html('State:').append(
    select.val(val));
}

function createVisibilitySelector(id = 'mdl_ipt_1', val = 'private') {
  let select = $('<select>').attr({'id': id, 'class': 'w3-input'});
  if (val === 'public') select.append(
    $(`<option>public</option>`, {'value': 'public'})).append(
    $(`<option>private</option>`, {'value': 'private'}));
...
function createRegionSelector(id = 'mdl_ipt_1', val = 'none') {
  ...
  return $('<label class=\'w3-bar-item\'>').html('Region:').append(
    select.val(val));
}

function createPeopleSelector(id = 'mdl_ipt_1') {
  let select = $('<select>').attr({'id': id, 'name': id, 'class': 'w3-input'});
  let people = _MODEL.getArray('p').sort(
...
function createChurchSelector(id = 'mdl_ipt_1') {
  let select = $('<select>').attr({'id': id, 'name': id, 'class': 'w3-input'});
  let church = _MODEL.getArray('c').sort(
    (p1, p2) => (p1['name'] < p2['name']) ? -1 : 1);
...
function createTextarea(label, id = 'mdl_ipt_1', val = '') {
  return $('<label class=\'w3-bar-item\'>').html(label).append($('<textarea>', {
    'id': id,
