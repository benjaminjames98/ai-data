...
function login_check (mysqli $mysqli) {
  ...
  if (isset($_SESSION['user_id'], $_SESSION['username'], $_SESSION['login_string'])) {
    ...
    if ($stmt = $mysqli->prepare("SELECT password FROM login_user WHERE `__pk_id` = ? LIMIT 1")) {
...
function permission_check (mysqli $mysqli, $permission) {
  // TODO replace login_check with permission_check
  if (!login_check($mysqli)) return false;
  if (!in_array($permission, ['per_users', 'per_churches', 'per_bread', 'per_education'])) return false;
